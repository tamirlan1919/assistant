...
commands = ['привет', 'открой файл', 'выключи комп', 'выруби компьютер', 'пока', 'покажи файл', 'покажи список команд',
            'открой vk', 'открой браузер', 'включи vk', 'открой интернет', 'открой youtube', 'включи музон', 'кто ты'
            'вруби музыку', 'очисти файл', 'найди', 'время', 'покажи погоду', 'ринх', 'ргэу', 'открой почту',
            'открой стату', 'покажи cтатистику','открой музыку', 'переведи', 'планы', 'на будущее',
            'что планируется', 'как тебя зовут','кто']
    ...
    Моя задача заключается четко выполнять требования ТамирлАна, ведь он меня сделал.
     Также в дальнейшем я буду работать как умный дом и я думаю, что ТамирлАн сделает оптимизацию моего кода,
      ведь он хорший кодер. В дальнейшем планируется распрделиться по всему дому с помощью
       микроконтроллера Ардуино. Планируется добавить к нему ИИ и сделать тело, а также разместить
        на таких площадках как Play Market.  
     '''
    engine.say(plans)
...
def web_search():  # осуществляет поиск в интернете по запросу (adress)
    global adress
    webbrowser.open('https://yandex.ru/yandsearch?clid=2028026&text={}&lr=11373'.format(
        adress))  # Поиск то, что находится в перем текст
...
def hello():  # функция приветствия
    global engine
    z = ["Jarvis здесь, Рад снова вас слышать!", 'Что вам угодно?', 'Привет. Чем-нибудь помочь?']  # Несколько вариаций
...
def weather():
    webbrowser.open(
        'https://yandex.ru/search/?text=%D0%BF%D0%BE%D0%B3%D0%BE%D0%B4%D0%B0%20%D0%B2%20%D1%80%D0%BE%D1%81%D1%82%D0%BE%D0%B2%D0%B5%20%D0%BD%D0%B0%20%D0%B4%D0%BE%D0%BD%D1%83&clid=2270455&banerid=6301000000%3A5f5bdc4eaac798001b7c8db4&win=454&&lr=39')  # Открывает погоду
...
def who():
    engine.say("я Jarvis, симуляция искусственого интелекта который использовал железный человек начиная с фильма "
...
cmds = {
    'привет': hello, 'выключи компьютер': shut,
    'пока': quit, 'покажи  cтатистику': pri_com, 'покажи список команд': show_cmds,
    'открой браузер': brows, 'включи vk': ovk,
    'открой youtube': youtube, 'включи музыку': music, 'открой vk': ovk,
    'включи музон': music, 'очисти файл': clear_analis, 'покажи файл': pri_com,
    'открой файл': pri_com, 'открой музыку': music,
    ...
    'покажи погоду': weather, 'ринх': rsue, 'ргэу': rsue, 'открой почту': mail, 'кто ты': who,
    'как тебя зовут': who, 'кто': who,
}
